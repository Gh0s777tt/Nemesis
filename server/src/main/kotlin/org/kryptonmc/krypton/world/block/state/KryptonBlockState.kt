...
class KryptonBlockState(
    ...
    // NOTE: ArticData's 1.19.3 schema dropped the per-block `material` field (Mojang removed Material in 1.20), so
    // BlockLoader assigns every block the STONE material — making material.solid/liquid/blocksMotion useless (solid
    // would be true even for air/water). Instead derive solidity from the per-block `blocksMotion` flag ArticData DOES
    // provide (stored as BlockProperties.hasCollision) and liquidity from the actual fluid state.
    override fun isSolid(): Boolean = block.properties.hasCollision

    override fun isLiquid(): Boolean = !asFluid().isEmpty()
    ...
    override fun blocksMotion(): Boolean = block.properties.hasCollision
