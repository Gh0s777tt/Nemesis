...
class KryptonFishingHook(world: KryptonWorld) : KryptonProjectile(world), FishingHook {
    ...
    override fun tick(time: Long) {
        super.tick(time)
        if (isRemoved()) return
        // Ballistic cast: horizontal launch velocity decaying with drag, plus gravity. Unlike a thrown potion the
        // bobber persists where it lands (until the rod is reeled in) instead of bursting.
        val v = velocity
    ...
    companion object {
        ...
        private const val HORIZONTAL_DRAG = 0.92
