...
abstract class KryptonEntity(final override var world: KryptonWorld) : BaseEntity, SerializableEntity, KryptonSender, Tickable {
    ...
    private var fallSpeed: Double = 0.0

    /**
     * Server-side gravity for server-owned entities (mobs, dropped items; NOT players, whose physics the client
     * simulates). Accelerates the entity downward toward the nearest solid block below it (up to a terminal
     * velocity), clamping to the ground top so it never sinks through, and accumulates [fallDistance] so that on
     * landing living entities take fall damage. No-ops while resting on the ground or over a void.
     */
    protected fun fallToGround() {
        val groundTop = groundTopBelow()
        if (groundTop == null) { // nothing solid below within range — over a void; don't fall, reset state
            fallSpeed = 0.0
            fallDistance = 0F
            return
        }
        if (position.y <= groundTop + GRAVITY_EPSILON) { // already resting on the ground
            onLanded()
            return
        }
        fallSpeed = minOf(fallSpeed + GRAVITY_ACCELERATION, GRAVITY_TERMINAL_VELOCITY)
        val newY = maxOf(groundTop, position.y - fallSpeed)
        fallDistance += (position.y - newY).toFloat()
        teleport(position.withCoordinates(position.x, newY, position.z))
        if (newY <= groundTop + GRAVITY_EPSILON) onLanded() // reached the ground this tick
    }

    /** The Y of the surface (top of the first solid block) below the entity, or null if none within range. */
    private fun groundTopBelow(): Double? {
        val bx = floor(position.x).toInt()
        val bz = floor(position.z).toInt()
        var by = floor(position.y).toInt()
        val minY = by - GRAVITY_SCAN_DEPTH
        while (by >= minY) {
            if (world.getBlock(bx, by, bz).isSolid()) return (by + 1).toDouble()
            by--
        }
        return null
    }

    /** Called when the entity comes to rest on the ground: deals fall damage to living entities, then resets. */
    private fun onLanded() {
        val distance = fallDistance
        fallDistance = 0F
        fallSpeed = 0.0
        if (distance > FALL_DAMAGE_THRESHOLD && this is KryptonLivingEntity) {
            health -= distance - FALL_DAMAGE_THRESHOLD // 1 damage per block fallen beyond the threshold
            if (health <= 0F) remove()
        }
    }
    ...
    companion object {
        ...
        private const val GRAVITY_ACCELERATION = 0.08      // downward accel per tick (velocity ramps up)
        private const val GRAVITY_TERMINAL_VELOCITY = 3.5  // max fall speed (blocks per tick)
        private const val GRAVITY_SCAN_DEPTH = 24          // how far down to look for ground before giving up
        private const val GRAVITY_EPSILON = 1.0E-3
        private const val FALL_DAMAGE_THRESHOLD = 3.0F     // blocks of fall before damage starts (1 dmg/block over)
    }
}
