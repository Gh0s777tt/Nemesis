...
//        }
        ...
        if (player.gameMode == GameMode.CREATIVE) return true // creative breaks don't drop items (vanilla)
        ...
        // Survival: the broken block drops its item as a pickable entity (closes the mine->pickup->craft loop).
        if (hasChanged) spawnBlockDrop(pos, state)
        return true
    }

    /**
     * Drops the broken [state]'s item as a [KryptonItemEntity] at the block's centre, collectable via ItemDropManager.
     * NOTE: drops in all game modes here (vanilla only drops in survival) — kept simple so the resource loop is testable.
     */
    private fun spawnBlockDrop(pos: Vec3i, state: KryptonBlockState) {
        val blockKey = KryptonRegistries.BLOCK.getKey(state.block).value()
        if (blockKey == "air") return
        val dropKey = BLOCK_DROPS.getOrDefault(blockKey, blockKey) // most blocks drop themselves; a few differ (stone->cobblestone)
        val itemType = KryptonRegistries.ITEM.get(Key.key(dropKey))
        if (itemType.key().value() == "air") return // block has no corresponding item
        val drop = KryptonItemEntity(player.world)
        drop.position = Position(pos.x + 0.5, pos.y + 0.5, pos.z + 0.5, 0F, 0F)
        drop.item = KryptonItemStack(itemType)
    ...
    companion object {
        ...
        // Blocks whose dropped item differs from the block itself; everything else drops its own item.
        private val BLOCK_DROPS = mapOf(
            "stone" to "cobblestone",
            "grass_block" to "dirt",
            "coal_ore" to "coal",
            "diamond_ore" to "diamond",
            "redstone_ore" to "redstone"
        )
    }
}
