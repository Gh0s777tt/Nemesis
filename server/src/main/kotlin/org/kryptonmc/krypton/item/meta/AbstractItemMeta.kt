...
abstract class AbstractItemMeta<I : ItemMeta>(val data: CompoundTag) : ItemMeta {
    ...
    companion object {
        ...
        @JvmStatic
        protected inline fun <R : Any> mapToList(data: CompoundTag, key: String, type: Int, mapper: (Tag) -> R?): ImmutableList<R> =
            mapTo(data, key, type, mapper) { ImmutableList.builder() }

        @JvmStatic
        protected inline fun <R : Any> mapToSet(data: CompoundTag, key: String, type: Int, mapper: (Tag) -> R?): ImmutableSet<R> =
            mapTo(data, key, type, mapper) { ImmutableSet.builder() }

        @JvmStatic
        protected inline fun <C : IC<R>, B : ICB<R>, R : Any> mapTo(data: CompoundTag, key: String, type: Int, mapper: (Tag) -> R?, builder: () -> B): C {
            val result = builder()
